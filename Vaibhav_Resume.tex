...
%--- HEADER ------------------------------------------------------------------
\begin{center}
  {\Large\bfseries\color{nameblue} Vaibhav Raheja}\\[2pt]
  \small
...
\begin{itemize}[leftmargin=1.2em, topsep=2pt, itemsep=0pt, parsep=0pt]
  \item Leading platform bring-up on NVIDIA Jetson: integrating ODrive motor controllers via CAN bus and establishing ROS2 driver interfaces from scratch.
  \item Defining hardware-software integration standards and Docker deployment workflows, enabling reproducible embedded deployments across robot units.
  \item Implementing LiDAR-based stop-by-obstacle detection as part of the autonomous navigation safety layer.
\end{itemize}

\workheader{EarthSense, Inc}{Robotics Engineer / Project Lead}{Aug 2024 – Mar 2026 $|$ Champaign, USA}
\begin{itemize}[leftmargin=1.2em, topsep=2pt, itemsep=0pt, parsep=0pt]
  \item Authored driver interfaces integrating Hesai LiDAR, PTZ cameras, and Seek thermal sensors into the ROS2 production stack across 3+ multi-site robot units.
  \item Built real-time hardware health monitoring and telemetry pipelines, achieving 40\% improvement in fault detection rates and 90\% fleet uptime.
  \item Shipped 35+ PRs across the motion control and autonomy stack in C++ and Python; stress-tested hardware at 40°C+ across multiple solar farm field deployments.
  \item Enabled low-latency teleoperation via an AWS Kinesis WebRTC proof of concept for remote fleet monitoring.
  \item Led a cross-functional team of 8+ engineers delivering an autonomous solar panel inspection platform across production solar farm deployments.
\end{itemize}
...
\begin{itemize}[leftmargin=1.2em, topsep=2pt, itemsep=0pt, parsep=0pt]
  ...
  \item Developed SOCRATES 2.0 on ROS with Velodyne LiDAR avoidance, OpenCV lane detection, GPS navigation, and ODrive S1 motor control for the competition.
\end{itemize}
